...
-   **Chromium Memory Auto-Restart**: The audio agent watches Chromium RSS every ~30s. When it exceeds `CHROMIUM_MAX_MB` (default 1200, override via env), the agent runs `pkill -f chromium` and the kiosk autostart relaunches a fresh tab. 5-minute cooldown between restarts prevents loops. This is the fast-leak safety net; the 04:00 cron is the slow-leak safety net.
