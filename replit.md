...
-   **Chromium Memory Auto-Restart**: The audio agent watches Chromium RSS every ~30s. When it exceeds `CHROMIUM_MAX_MB` (default 2500 MB — high to avoid mid-set restarts; set to 0 to disable) AND no audio sink is currently in `RUNNING` state (i.e. a song is playing), the agent runs `pkill -f chromium` and the kiosk autostart relaunches a fresh tab. 30-min cooldown between restarts prevents loops. The "no music playing" guard means restarts happen between songs/sets, not mid-song. This is the catastrophic-leak safety net; the 04:00 cron is the regular slow-leak safety net.
