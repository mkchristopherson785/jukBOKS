...
-   **Guest Ranking / Gamification**: Guests earn ranks based on total upvotes received on their song requests. Ranks: Wallflower (0-4, no hat), Crowd Pleaser (5-14, party hat), Vibe Curator (15-29, beret), Beat Dropper (30-49, backwards cap), Hitmaker (50-99, top hat), Jukebox Hero (100+, crown). Components: `GuestRankBadge` (inline hat icon next to requester names in queue), `GuestRankCard` (full card with progress bar shown on party/search pages). Rankings aggregated server-side via `getGuestRankings()` in storage, returned in party API response. Case-insensitive name matching for display.
